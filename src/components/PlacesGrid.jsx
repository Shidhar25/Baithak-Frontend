...
export default function PlacesGrid({ overview }) {
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4" id="placesGrid">
        {items.map((p, idx) => (
          <div
            key={idx}
            className={`p-4 border rounded shadow-sm text-center ${p.personName ? 'bg-red-100' : 'bg-green-100'}`}
          >
            ...
            <p className="text-xs">{p.personName ? `Assigned: ${p.personName}` : 'Available'}</p>
          </div>
        ))}
        {items.length === 0 && (
